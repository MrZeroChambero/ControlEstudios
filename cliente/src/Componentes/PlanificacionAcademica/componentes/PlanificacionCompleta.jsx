...
export const PlanificacionCompleta = ({
  ...
  return (
    <div className="space-y-4">
      {selectedCompetencyGroups.map((grupo) => (
        <article key={grupo.clave} className={competenciaCardClase}>
          <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
            ...
            <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">
              {Array.isArray(grupo.competencias)
                ? grupo.competencias.length
                : 0}{" "}
              competencia(s)
            </span>
          </header>
          <div className="space-y-4 px-4 py-4">
            {(Array.isArray(grupo.competencias) ? grupo.competencias : []).map(
              (competencia) => (
                <div
                  key={competencia.id}
                ...
                >
                  ...
                  <div className="space-y-2 border-t border-slate-200 pt-3">
                    ...
                    {!(
                      Array.isArray(competencia.indicadores) &&
                      competencia.indicadores.length
                    ) ? (
                      <p className="text-sm text-slate-500">
