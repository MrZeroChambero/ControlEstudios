...
export const ComponentesAprendizajes = () => {
  return <MenuPrincipal Formulario={MenuComponentesAprendizaje} />;
};

const MenuComponentesAprendizaje = () => {
  ...
  const handleStatusChange = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.patch(`${API_URL}/${id}/estado`, {}, { withCredentials: true });
          ...
          Swal.fire('Error', 'No se pudo cambiar el estado del componente.', 'error');
        }
      }
    });
  };
  ...
  const openModal = (componente = null, viewMode = false) => {
    ...
    setFormData(componente ? { nombre_componente: componente.nombre_componente } : { nombre_componente: '' });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e) => {
    ...
    enviarComponenteAprendizaje(formData, currentComponente, API_URL, refetchData, closeModal);
  };
    ...
    <>
      <div className="p-6 bg-white rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Gestión de Componentes de Aprendizaje</h2>
          <button
            onClick={() => openModal()}
