...
trait OperacionesBDTrait
{
  ...
  protected function ejecutarAccion(PDO $conexion, string $sql, array $parametros = []): bool
  {
    try {
      ...
      throw new Exception('Error al ejecutar la accion: ' . $excepcion->getMessage(), 0, $excepcion);
    }
  }

  protected function ejecutarValor(PDO $conexion, string $sql, array $parametros = [])
  {
    try {
      $sentencia = $conexion->prepare($sql);
      $sentencia->execute($parametros);
