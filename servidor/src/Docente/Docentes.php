...
class Docente
{
  ...
  /**
   * Valida los datos del objeto Docente usando Valitron.
  ...
  /**
   * Crea un nuevo docente en la base de datos, con validación previa.
  ...
  public function crear(PDO $pdo)
  {
    ...
    try {
      ...
      if ($e->getCode() == 23000) {
        return ['rif' => ['El RIF ya se encuentra registrado.']];
      }
      return false;
    }
  }
  ...
  /**
   * Actualiza los datos de un docente, con validación previa.
  ...
  public function actualizar(PDO $pdo)
  {
    ...
    try {
      $sql = "UPDATE personal SET codigo_rac=?, cargo=?, cargo_tipo_personal=?, fecha_contratacion=?, departamento=?, nivel_academico=?, anios_servicio=?, horas_academicas=?, horas_adm=?, turno_atiende=?, grado_imparte=?, seccion_imparte=?, situacion_trabajador=?, observacion=?, rif=?, estado_civil=?, etnia_religion=?, numero_hijos=?, cantidad_hijas=?, cantidad_hijos_varones=? WHERE id_personal=?";
      $stmt = $pdo->prepare($sql);
      return $stmt->execute([
        $this->codigo_rac,
        $this->cargo,
  ...
  /**
   * Consulta los datos de un docente por su ID.
   * @param PDO $pdo Objeto de conexión.
   * @param int $id ID del personal a consultar.
   * @return object|false Un objeto Docente con sus datos o false si no se encuentra.
   */
  public static function consultar(PDO $pdo, $id)
  {
    try {
      ...
      if ($data) {
        ...
        $docente->id_personal = $data['id_personal'];
        return $docente;
      }
      return false;
    } catch (Exception $e) {
  ...
  /**
   * Verifica la existencia de un docente por su ID.
   * @param PDO $pdo Objeto de conexión.
   * @param int $id ID del personal a verificar.
