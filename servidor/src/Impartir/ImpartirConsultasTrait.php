...
trait ImpartirConsultasTrait
{
  use ImpartirHelpersTrait;
  ...
  protected function obtenerAulasPorAnio(PDO $conexion, int $anioId): array
  {
    ...
    return array_map(function (array $fila): array {
      ...
      $fila['grado'] = (int) $fila['grado'];
      $fila['seccion'] = $fila['seccion'];
      return $fila;
    }, $registros);
  }

  protected function obtenerAulaPorId(PDO $conexion, int $aulaId): ?array
  {
    $sql = "SELECT a.id_aula,
                   a.fk_anio_escolar,
                   a.fk_grado_seccion,
  ...
  public function obtenerMomentosPorAnio(PDO $conexion, int $anioId): array
  {
  ...
  public function obtenerMomentosActivos(PDO $conexion, int $anioId): array
  {
  ...
  public function personalTieneAsignacionActiva(PDO $conexion, int $anioId, int $personalId, int $aulaIdActual): bool
  {
    $sql = 'SELECT 1
            FROM imparte i
  ...
  public function obtenerComponentesPorIds(PDO $conexion, array $componentes): array
  {
  ...
  public function obtenerAreasConComponentes(PDO $conexion): array
  {
  ...
  public function obtenerAsignacionesEspecialistas(PDO $conexion, array $aulaIds): array
  {
    if (empty($aulaIds)) {
      return [];
    }
  ...
  protected function indexarComponentesPorId(array $areas): array
  {
    $catalogo = [];
    foreach ($areas as $area) {
      foreach ($area['componentes'] as $componente) {
        $catalogo[(int) $componente['id']] = $componente;
      }
    }
  ...
  public function construirResumenGestion(PDO $conexion, array $anio, array $momentos, array $aulas, array $areas): array
  {
    ...
    $componentesCatalogo = $this->indexarComponentesPorId($areas);
    ...
    $aulasRespuesta = array_map(function (array $aula) use ($docentes, $especialistas, $componentesCatalogo): array {
      ...
      $especialistasAsignados = array_values($especialistas[$aulaId] ?? []);
      $componentesDocente = $docente['componentes'] ?? [];

      $pendientesEspecialistas = 0;
      foreach ($componentesCatalogo as $componenteId => $componente) {
        if (!empty($componente['requiere_especialista']) && !isset($especialistas[$aulaId][$componenteId])) {
      ...
      return [
        'id' => $aulaId,
        'grado' => (int) ($aula['grado'] ?? 0),
        'seccion' => $aula['seccion'] ?? '',
        'estado' => $aula['estado'] ?? null,
        'docente' => $docente,
        'componentes_docente' => array_values($componentesDocente),
        'especialistas' => $especialistasAsignados,
        'pendientes' => [
    ...
    $this->ordenarAulasPorGradoSeccion($aulasRespuesta);
